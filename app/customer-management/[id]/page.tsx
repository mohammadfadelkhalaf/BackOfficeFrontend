...
const ViewSingleUser = ({ params }) => {
    ...
    useEffect(() => {
        const getUserPayment = async () => {
            const response = await axios.get(`https://localhost:7098/api/Orders/GetOrderDetailsByUserId/${id}`);
        ...
        const getUserCourse = async () => {
            const response = await axios.get(`https://localhost:7098/api/UserCourses/GetCourseDetailsByUserId/${id}`);
            if (response.status !== 200) {
                return;
            }
            ...
            <Box sx={{ width: '100%', fontSize: "1.6rem" }}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                        <Tab sx={{ fontSize: "1.6rem" }} label="Payment History" {...a11yProps(0)} />
                        <Tab sx={{ fontSize: "1.6rem" }} label="Course History" {...a11yProps(1)} />
                    </Tabs>
                </Box>
                    ...
                    <Box sx={{ width: '100%', maxHeight: "25em", overflowY: "auto" }}>
                        <Paper sx={{ width: '100%' }}>
                            <TableContainer>
                                ...
                                >
                                    <EnhancedTableHeadOne
                                        rowCount={userPayment.length}
                                    />
                                    <TableBody>
                                        {userPayment?.map((row) => {
                                            return (
                                                <TableRow
                                                    ...
                                                    sx={{ cursor: 'pointer' }}
                                                >
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.batchName}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.paidAmount}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.startDate}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.maxMemberCount}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.course?.title}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.course?.price}</TableCell>
                                                    <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row?.batch?.course?.author}</TableCell>
                        ...
                        <Paper sx={{ width: '100%' }}>
                            <TableContainer>
                                <Table
                                    ...
                                    aria-labelledby="tableTitle"
                                >
                                    <EnhancedTableHeadTwo
                                        rowCount={userCourse.length}
                                    />
                                    <TableBody>
                                        {userCourse?.map((row) => {
                                            return (
