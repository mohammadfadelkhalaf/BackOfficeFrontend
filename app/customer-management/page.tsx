...
const CustomerManagement = () => {
    ...
    const uploadImage = async (imageFile: any) => {
        ...
        try {
            const response = await axios.post('https://localhost:7098/api/FileUploader', formData, {
                headers: {
    ...
    const handleDelete = async (id: string) => {
        console.log(id);
        const response = await axios.delete(`https://localhost:7098/api/Users/${id}`);
    ...
    const handleUpdateUser = async (event: any) => {
        ...
        try {
            const response = await axios.put(`https://localhost:7098/api/Users/${id}`, userObj);
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - 10) : 0; // rows.length
    ...
    return (
        <div className="shadow-xl max-w-screen-xl mx-auto mt-32 overflow-hidden mb-20">
                                ...
                                {rows?.map((row) => {
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.lastName}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.userName}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.email}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.bio}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.profileImage}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>{row.currentBalance}</TableCell>
                                            <TableCell align="center" sx={{ fontSize: "1.6rem" }}>
                                                <div className="flex justify-center items-center">
                                                    <button title="Edit" onClick={() => handleEdit(row)}>
            ...
            >
                <DialogTitle id="scroll-dialog-title" className="!text-[2rem] !font-semibold">Edit User</DialogTitle>
                <DialogContent dividers={scroll === 'paper'}>
                    ...
                    >
                        <form className="batch-form w-full p-5 mx-auto" onSubmit={(e) => handleUpdateUser(e)}>
                            <div className="flex">
                            ...
                            <div className="flex">
                                <div className="flex flex-col w-full my-3 mx-4">
                                    ...
                                    <input type="text" name="userName" defaultValue={user.userName} placeholder="User Name" />
                                </div>

                                <div className="flex flex-col w-full my-3 mx-4">
                                    <label>Bio</label>
                                    <input type="text" defaultValue={user.bio} placeholder="Bio" name="bio" />
                                </div>
                            ...
                            <div className="flex">
                                ...
                                <div className="flex flex-col w-full my-3 mx-4">
                                    ...
                                    <input type="file" name="profileImage" onChange={(e) => handleImageChange(e)} />
                                </div>
                            </div>

                            <div className="flex flex-col w-full my-3 mx-4">
                                <label>Current Balance</label>
                                <input type="number" name="currentBalance" defaultValue={user.currentBalance} placeholder='Current Balanace' />
                            </div>

                            <div className="flex">
                                <div className="flex flex-col w-full my-3 mx-4">
                                    <FormLabel id="demo-row-radio-buttons-group-label" className="!text-3xl !text-black">Is Email Confirmed</FormLabel>
                                    <RadioGroup
                                        row
                                ...
                                <div className="flex flex-col w-full my-3 mx-4">
                                    <div className="w-[10rem] h-[10rem] overflow-hidden">
                                        <img src={imagePreviewUrl ? imagePreviewUrl : user.profileImage} alt="profile Image" className="object-cover w-full h-full" />
                                    </div>
                                </div>
                            </div>

                            <div className="submit-btn">
                                <input type="submit" className="cursor-pointer" value="Update User" />
                            </div>
                        </form>
                    </DialogContentText>
                </DialogContent>
